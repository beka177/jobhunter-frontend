...
const ApplicationsList = ({ user, onNavigate, onOpenChat }) => {
  ...
  const notifyChat = async (app, newStatus) => {
    const body = t(newStatus === 'accepted' ? 'apps.notify.accepted' : 'apps.notify.rejected', { title: app.vacancy_title });
    try {
  ...
  const handleStatusChange = async (appId, newStatus) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        toast.success(t(newStatus === 'accepted' ? 'apps.toast.accepted' : 'apps.toast.rejected'));
      } else {
        const err = await response.json().catch(() => ({}));
        toast.error(t('apps.toast.update_error') + ': ' + (err.message || t('common.unknown_error')));
      }
    } catch (error) {
  ...
  return (
    <div className="space-y-6 mt-6 max-w-7xl mx-auto">
      ...
      <h2 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-6">{t('apps.title')}</h2>
      ...
      {applications.length === 0 ? (
         <div className="text-center py-16 bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 mt-6">
          ...
          <h3 className="mt-4 text-lg font-medium text-gray-900 dark:text-white">{t('apps.empty.title')}</h3>
          <p className="mt-2 text-gray-500 dark:text-gray-400">{t('apps.empty.desc')}</p>
        </div>
      ) : (
                ...
                <div className="px-6 py-6 sm:px-8">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{t('apps.candidate')}: <span className="font-semibold text-gray-900 dark:text-white">{app.seeker_name}</span></p>
                      </div>
                    </div>

                    <div className="flex-shrink-0 flex items-center">
                      {app.status === 'pending' && <span className="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-400"><Clock className="w-4 h-4 mr-1.5"/> {t('apps.status.pending')}</span>}
                      {app.status === 'accepted' && <span className="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-400"><CheckCircle className="w-4 h-4 mr-1.5"/> {t('apps.status.accepted')}</span>}
                      {app.status === 'rejected' && <span className="px-3 py-1 inline-flex text-sm leading-5 font-semibold rounded-full bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-400"><XCircle className="w-4 h-4 mr-1.5"/> {t('apps.status.rejected')}</span>}
                    </div>
                  </div>
                  ...
                  <div className="mt-4 flex items-center justify-between">
                    <div className="flex items-center text-sm text-gray-500 dark:text-gray-400">
                      ...
                      <p>{t('apps.date_sent')}: {new Date(app.created_at).toLocaleDateString(lang === 'kk' ? 'kk-KZ' : 'ru-RU')}</p>
                    </div>
            ...
            <div className="inline-block align-bottom bg-white dark:bg-gray-800 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
              <div className="bg-white dark:bg-gray-800 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                <div className="flex justify-between items-start mb-4 border-b dark:border-gray-700 pb-4">
                  <h3 className="text-2xl font-bold text-gray-900 dark:text-white">{t('apps.modal.title')}</h3>
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                     <div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded">
                        <span className="text-xs text-gray-500 dark:text-gray-400 block">{t('apps.modal.desired_position')}</span>
                        <span className="font-semibold text-gray-900 dark:text-white">{selectedApp.profession || t('apps.profession_unspecified')}</span>
                     </div>
                     <div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded">
                        <span className="text-xs text-gray-500 dark:text-gray-400 block">{t('apps.modal.city')}</span>
                        <span className="font-semibold text-gray-900 dark:text-white flex items-center">
                           <MapPin className="w-3 h-3 mr-1" /> {selectedApp.city || t('common.not_specified')}
                        </span>
                     </div>
                  </div>

                  <div className="border-t dark:border-gray-700 pt-4">
                    <h4 className="font-bold text-gray-800 dark:text-gray-200 flex items-center mb-2"><User className="w-4 h-4 mr-2" /> {t('apps.modal.personal')}</h4>
                    <div className="grid grid-cols-2 gap-y-2 text-sm text-gray-700 dark:text-gray-300">
                        <p>{t('slist.filter.gender')}: <span className="font-medium">{selectedApp.gender === 'male' ? t('slist.field.gender_male') : t('slist.field.gender_female')}</span></p>
                        <p>{t('slist.field.birthday')}: <span className="font-medium">{selectedApp.birthday || '-'}</span></p>
                        <p>{t('slist.field.citizenship')}: <span className="font-medium">{selectedApp.citizenship || '-'}</span></p>
                        <p className="flex items-center"><Phone className="w-3 h-3 mr-1"/> {selectedApp.phone || '-'}</p>
                    </div>
                  </div>

                  <div className="border-t dark:border-gray-700 pt-4">
                    <h4 className="font-bold text-gray-800 dark:text-gray-200 flex items-center mb-2"><GraduationCap className="w-4 h-4 mr-2" /> {t('apps.modal.education')}</h4>
                  ...
                  <div className="border-t dark:border-gray-700 pt-4">
                    <h4 className="font-bold text-gray-800 dark:text-gray-200 flex items-center mb-2"><Globe className="w-4 h-4 mr-2" /> {t('apps.modal.skills')}</h4>
                    <div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded text-sm text-gray-700 dark:text-gray-300 border border-gray-100 dark:border-gray-600">{selectedApp.skills || t('apps.modal.skills_unspecified')}</div>
                  </div>
                </div>
              </div>
